...
<? document($data) ?>
<? endAdmin($menu, true) ?>
<? module("doc:read:catalog", array('parent' => $id, 'type' => 'product')) ?>
<? } ?><? //	Template doc_page_default loaded from  _sites/dt-ekb.ru/_modules/_catalog/template.doc_page_default.php3 ?>
...
<? displayThumbImage($title = docTitleImage($id), array(450, 350), ' class="thumb"', '', $title) ?>
...
<? endAdmin($menu, true) ?>
<? } ?><? //	Template doc_read_catalog loaded from  _sites/dt-ekb.ru/_modules/_product/template.doc_read_catalog.php3 ?>
...
?><? if(isset($name)) echo htmlspecialchars($name) ?> <b><? if(isset($val)) echo $val ?></b>
<? } ?>
</big></div>
    <h4><? $split = ''; foreach($price as $name => $val){ ?><? if(isset($split)) echo htmlspecialchars($split) ?><? if(isset($name)) echo htmlspecialchars($name) ?> <span class="price"><? if(isset($val)) echo $val ?></span><? $split = ' + '; } ?></h4>
<div class="property">
<? foreach($p as $name => &$val){
	if ($name[0] == ':'){
		echo $val; continue;
...
<? function operator_contacts($val, $data){ ?>
