...
?>
<? // Module module_doc_menu loaded from  _modules/_module.doc/_pages/_reads/_menu/module_doc_menu.php ?>
﻿<?
...
?>
<? // Module module_fs_common loaded from  _sites/fs/_modules/module_fs_common.php ?>
<? function masterDate(&$data){
	$date	= makeDate($data['masterDate']);
	if (!$date) return;
	
	$w		= explode(',', 'Воскресение,Понедельник,Вторник,Среда,Четверг,Пятница,Суббота');
	$w		= $w[(int)date('w', $date)];
	$d		= date('d', $date);
	$m		= explode(',', ',Января,Февраля,Марта,Апреля,Мая,Июня,Июля,Августа,Сентября,Ноября,Декабря');
	$m		= $m[(int)date('m', $date)];
	$t		= date('H:i',$date);
	return "<big>$d</big> $m <big>$t</big> <b>$w</b>";
}
function gallery_config($val, &$data){
	$data['size']	= array(232, 240);
}
?>
