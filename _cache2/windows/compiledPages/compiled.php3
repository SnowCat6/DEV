...
?><? //	Template module_advFeedback loaded from  _sites/windows/_modules/_advFeedback/template.module_advFeedback.php3 ?>
<? function module_advFeedback($val)
{
	module('script:jq');
	module('page:style', 'adv.css');
	
	if (!beginCache($cache = "adv/$val")) return;
	@$adv = file_get_contents(localCacheFolder."/siteFiles/adv/$val/index.php");
	if (!$adv) return endCache($cache);
...
?>
<div class="advFeedback"><? eval('?>'.$adv.'<?')?><div class="advSeek"></div></div>
<script>
...
$(function(){
	...
	$(".advFeedback .adv").each(function(){
		$(".advFeedback .advSeek").append('<a href="#" />');
	});

	$(".advFeedback .advSeek a").hover(function(){
...
function nextAdvFeedback(){
	setFeedbackAdv($(".advFeedback .advSeek a.current").index()+1);
	advFeedbackTimer = setTimeout(nextAdvFeedback, 5000);
}
function setFeedbackAdv(ndx, bNoFade)
{
	ndx = ndx % $(".advFeedback .advSeek a").length;
	if ($(".advFeedback .advSeek a.current").index() == ndx) return;
	...
	if (bNoFade != true){
		...
		$(".advFeedback .advSeek a.current").removeClass("current");
	}
	
	$($(".advFeedback .adv")[ndx]).addClass('current').fadeIn();
	$($(".advFeedback .advSeek a")[ndx]).addClass("current");
}
</script>
<? endCache($cache) ?>
<? } ?><? //	Template doc_read_feedback loaded from  _sites/windows/_modules/_pages/template.doc_read_feedback.php3 ?>
<?
