...
<? endAdmin($menu, true) ?><? event('document.comment',	$id)?><? } ?><? //	Template doc_read_catalog loaded from  _sites/tis/_modules/template.doc_read_catalog.php3 ?>
<?
...
function doc_read_catalog(&$db, $val, &$search)
{
	...
	if (!$pages) $pages = 25;
	$p		= dbSeek($db, $pages, array('search' => $search));
?><? module("display:sort"); ?><? if(isset($p)) echo $p ?>
<table width="100%" cellpadding="0" cellspacing="0" class="productTable">
<? while($data = $db->next()){
	$id			= $db->id();
	$menu		= doc_menu($id, $data);
	$url		= getURL($db->url());
	$price		= docPriceFormat($data);
	$class		= $db->ndx % 2 == 0?' class="rowAlt"':'';
?>
<tr <? if(isset($class)) echo $class ?>>
	<th width="64"><? $module_data = array(); $module_data["mask"] = "design/productMask.png"; moduleEx("doc:titleImage:$id:mask", $module_data); ?></th>
    <th>
    <h3><a href="<? if(isset($url)) echo htmlspecialchars($url) ?>" id="ajax" title="<? if(isset($data["title"])) echo htmlspecialchars($data["title"]) ?>"><? if(isset($data["title"])) echo htmlspecialchars($data["title"]) ?></a></h3>
    <? $module_data = array(); $module_data["id"] = "$id"; $module_data["group"] = "productSearch2"; moduleEx("prop:read:plain", $module_data); ?>
    </th>
    <td><? if(isset($price)) echo $price ?> руб.</td>
</tr>
<? }//	foreach ?>
</table>
...
<? return true; } ?><? //	Template module_bask_compact loaded from  _sites/tis/_modules/_bask/template.module_bask_compact.php3 ?>
<?
...
<div class="baskAvalible">В корзине <b><? if(isset($count)) echo htmlspecialchars($count) ?></b> шт.</div>
<a href="<? module("url:bask"); ?>">Оформить заказ</a>
</div>
    <? }else{ ?>
<div class="bask compact">
<div class="baskTitle"><a href="<? module("getURL:bask"); ?>" id="ajax">Корзина пуста:</a></div>
<div class="baskAvalible">Перейдите в каталог,<br />
выберите требуемый товар и<br />
добавьте его в корзину.
</div>
</div>
    <? } ?><? } ?><? //	Template doc_sort loaded from  _sites/tis/_modules/_sort/template.doc_sort.php3 ?>
<? function doc_sort($db, $val, &$search)
{
	if ($search['type'] != 'product') return;
	
	$order	= array();
	$order['назазванию']	= 'name';
	$order['цене']			= 'price';
	$orderDefault			= 'name';
	...
	$thisValue	= $search[':order'];
	if (!isset($docSort[$thisValue])) $thisValue = $orderDefault;
	$search[':order']	= $thisValue;
	
	/*******************************************/
	$pagesDefault	= '25';
	$pages			= getCacheValue('docPages');
	$thisPages		= $search[':pages'];
	if (!isset($pages[$thisPages])) $thisPages = $pagesDefault;
	$search[':pages'] = $thisPages;
?>
<div class="sort">
	<div class="order">Сортировать по: <span class="holder">
<?
$first = ' id="first"';
foreach($order as $name => &$val)
{
	$class = '';
	if (trim($thisValue, '-') == $val){
		$class = 'current';

		if ($thisValue != $val) $class .= ' reverse';
		else $val = "-$val";
	...
	$s	= $array;
	$s[':order']	= $val;
	if ($val == $orderDefault) unset($s[':order']);
	$s	= makeQueryString($s, 'search');
	$url= getURL('#', $s);
?>
<a href="<? if(isset($url)) echo $url ?>"<? if(isset($class)) echo $class ?><? if(isset($first)) echo $first ?>><span><? if(isset($name)) echo htmlspecialchars($name) ?></span></a>
<? $first = ''; } ?>
	</span></div>
	<div class="pages"><span class="holder">

<?
$first = ' id="first"';
foreach($pages as $name => &$val)
{
	$class= $thisPages	== $name?' class="current"':'';
	
	$s	= $array;
	$s[':order']	= $thisValue;
	if ($thisValue == $orderDefault) unset($s[':order']);
	
	$s[':pages']	= $name;
	if ($val == $pagesDefault) unset($s[':pages']);
...
<? } ?><? //	Template user_compact loaded from  _sites/tis/_modules/_user/template.user_compact.php3 ?>
<? function user_compact($db, $val, $data)
{
	$userID	= userID();
	if (!$userID) return module('user:loginForm');
?>
<div class="cabinet">
<div class="head">
    <b>Личный кабинет:</b>
...
<div class="info">
    <h3><? module("user:name"); ?></h3>
    <div class="menu inline">
    <a href="<? module("url:login"); ?>" id="first">Заказы</a>
    <a href="<? module("url:user_edit_$userID"); ?>">Настройки</a>
    </div>
</div>
</div>
<? } ?>