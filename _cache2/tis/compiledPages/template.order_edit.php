...
?><? module("page:style", 'order.css') ?><? $module_data = array(); $module_data[] = "Редактирование заказа"; moduleEx("page:title", $module_data); ?>
<form action="<? module("getURL:order_edit$id"); ?>" method="post" class="ajaxFrom ajaxReload">
...
<? } ?>
  ...
  <tr>
    ...
    <td><textarea name="order[orderNote]" cols="" rows="4" class="input w100"><? if(isset($data["orderNote"])) echo htmlspecialchars($data["orderNote"]) ?></textarea></td>
  </tr>
<? foreach($orderData as $type => $val){ ?><? foreach($val as $name => $value){?>
  <tr>
