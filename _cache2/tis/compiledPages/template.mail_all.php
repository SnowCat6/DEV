...
//	if (!hasAccessRole('admin,developer,writer,manager')) return;
	...
	$db->open();
	$p = dbSeek($db, 15);
?><? module("page:style", 'admin.css') ?><? module("page:style", 'baseStyle.css') ?><? $module_data = array(); $module_data[] = "Отправленные письма"; moduleEx("page:title", $module_data); ?><? if(isset($p)) echo $p ?>
