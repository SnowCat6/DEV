...
function import_ui($val, &$data)
{
	...
	//	Вывести данные
?>
<form action="<? module("getURL:import"); ?>"  method="post" enctype="multipart/form-data">
...
<? function importUI($val, $data)
{
	$hasWorking = false;
?>
<? $module_data = array(); $module_data[] = "Импорт файлов"; moduleEx("page:title", $module_data); ?>
<form action="<? module("getURL:import"); ?>" method="post">
...
<tr>
  <td valign="top">
  <input type="submit" name="import[delete][<? if(isset($file)) echo htmlspecialchars($file) ?>]" class="button" value="X" />
  </td>
    <td valign="top" nowrap="nowrap">
<div title="<? if(isset($path)) echo htmlspecialchars($path) ?>"><? if(isset($file)) echo htmlspecialchars($file) ?></div>
<div><?= date('<b>d.m.Y</b> H:i:s', filemtime($path))?></div>
    </td>
    <td valign="top" nowrap="nowrap">
  <div><b><? if(isset($process["percent"])) echo htmlspecialchars($process["percent"]) ?>%</b> <?= round($process['offset']/1024, 2)?> кб. / <?= round($process['size']/1024, 2)?> кб.</div>
  <div><a href="<? $module_data = array(); $module_data["file"] = "$file"; moduleEx("getURL:import_log", $module_data); ?>" id="ajax"><? if(isset($processDate)) echo $processDate ?> <b><? if(isset($workTime)) echo htmlspecialchars($workTime) ?> сек.</b></a></div>
    </td>
    <td align="right" valign="top" nowrap="nowrap">
<? switch($process['status']){ ?>
<? case 'working': $hasWorking = true; ?>
<input type="submit" name="import[continue][<? if(isset($file)) echo htmlspecialchars($file) ?>]" class="button" value="Продолжить" />
<input type="submit" name="import[cancel][<? if(isset($file)) echo htmlspecialchars($file) ?>]" class="button" value="X" />
<? break ?>
<? case 'complete': ?>
<input type="submit" name="import[import][<? if(isset($file)) echo htmlspecialchars($file) ?>]" class="button" value="Повторить" />
<? break ?>
<? default: ?>
<input type="submit" name="import[import][<? if(isset($file)) echo htmlspecialchars($file) ?>]" class="button" value="Импортировать" />
<? } ?>
    </td>
</tr>
<? } ?>
