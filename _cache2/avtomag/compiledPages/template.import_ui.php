<? function import_ui($val, &$data)
{
	...
	m('script:jq');
?>
<form action="<? module("url:import"); ?>" method="post" enctype="multipart/form-data">
...
<form action="<? module("url:import"); ?>" method="post" id="reload">
<div><? importInfo() ?></div>
<script>
...
?>
  <tr>
    <td>
		<input type="submit" class="button" name="doDelete[<? if(isset($name)) echo htmlspecialchars($name) ?>]" value="x" />
    </td>
    ...
    <td nowrap="nowrap"><? if(isset($status)) echo htmlspecialchars($status) ?></td>
    <td nowrap="nowrap"><? if(isset($progress)) echo $progress ?></td>
    <td nowrap="nowrap"><? if(isset($lockInfo)) echo $lockInfo ?></td>
    <td nowrap="nowrap"><?
...
if (!$statistic) $statistic = array();
    ...
    <td nowrap="nowrap">
<? if ($synch->logRead(1)){ ?>
    <a href="<? $module_data = array(); $module_data["name"] = "$name"; moduleEx("url:import_log", $module_data); ?>" id="ajax">лог импорта</a>
<? } ?>
    </td>
    <td nowrap="nowrap"><? switch($status){ ?><? case '---': ?>
<div><input type="submit" class="button w100" name="doSynch[<? if(isset($name)) echo htmlspecialchars($name) ?>]" value="Начать" /></div>
<? break; ?><? case 'complete': ?>
<div><input type="submit" class="button w100" name="doSynchRepeat[<? if(isset($name)) echo htmlspecialchars($name) ?>]" value="Повторить" /></div>
<? break; ?><? default: ?>
<div>
<input type="submit" class="button" name="doSynch[<? if(isset($name)) echo htmlspecialchars($name) ?>]" value="Продолжить" />
<input type="submit" class="button" name="doCancel[<? if(isset($name)) echo htmlspecialchars($name) ?>]" value="||" />
</div>
<? break; ?><? } ?></td>
  </tr>
<? } ?>
