...
function prop_edit($db, $val, $data){
	...
	$data = $db->openID($id);
?><? $module_data = array(); $module_data[] = "Изменение свойства"; moduleEx("page:title", $module_data); ?>
<form action="<? module("getURL:property_edit_$id"); ?>" method="post" class="admin ajaxForm ajaxReload">
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td valign="top"><table width="100%" border="0" cellpadding="0" cellspacing="0">
      <tr>
        <td nowrap="nowrap">Название</td>
        <td width="100%"><input type="text"name="property[name]" class="input w100" value="<? if(isset($data["name"])) echo htmlspecialchars($data["name"]) ?>" /></td>
      </tr>
      <tr>
        <td nowrap="nowrap">Формат*</td>
        <td><input type="text" name="property[format]" class="input w100" value="<? if(isset($data["format"])) echo htmlspecialchars($data["format"]) ?>" /></td>
      </tr>
      <tr>
...
?>
    ...
    <td><input type="text" name="property[sort]" class="input" value="<? if(isset($data["sort"])) echo htmlspecialchars($data["sort"]) ?>" /></td>
  </tr>
</table>
</td>
      </tr>
...
?>
<div>
<input name="property[group][<? if(isset($name)) echo htmlspecialchars($name) ?>]" type="hidden" value="" />
<label><input type="checkbox" name="property[group][<? if(isset($name)) echo htmlspecialchars($name) ?>]" value="<? if(isset($name)) echo htmlspecialchars($name) ?>"<? if(isset($class)) echo $class ?> /> <? if(isset($value)) echo htmlspecialchars($value) ?></label>
</div>
<? } ?>
<div>
...
<div>
  <table width="100%" border="0" cellspacing="0" cellpadding="0" class="table">
    ...
    <tr>
      <td valign="top"><textarea name="property[note]" rows="6" class="input w100"><? if(isset($data["note"])) echo htmlspecialchars($data["note"]) ?></textarea></td>
      <td valign="top">&nbsp;</td>
      <td valign="top">
...
?><option value="<? if(isset($query)) echo htmlspecialchars($query) ?>"<? if(isset($class)) echo $class ?>><? if(isset($name)) echo htmlspecialchars($name) ?></option>
<? } ?>
</select>
<div><textarea name="property[query]" rows="4" class="input w100"><? if(isset($data["query"])) echo htmlspecialchars($data["query"]) ?></textarea></div>
      </td>
    </tr>
    <tr>
    ...
    <tr>
      <td valign="top"><textarea name="property[alias]" rows="5" class="input w100"><? if(isset($data["alias"])) echo htmlspecialchars($data["alias"]) ?></textarea>
        <br />
        В каждой строчке по одному названию, при обноружении свойств с таким названием они будут объеденены</td>
      <td valign="top">&nbsp;</td>
      <td valign="top"><textarea name="property[values]" rows="5" class="input w100"><? if(isset($data["values"])) echo htmlspecialchars($data["values"]) ?></textarea>
        <br />
