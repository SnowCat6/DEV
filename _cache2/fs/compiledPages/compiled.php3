...
<? function doc_page_catalog(&$db, &$menu, &$data){
	$id = $db->id();
?><? module("page:style", 'baseStyle.css') ?><? beginAdmin() ?><? document($data) ?><? endAdmin($menu, true) ?><? $search = module("doc:search:$id", getValue('search')) ?>
<div class="product list">
<?	if ($search){ ?>
<h2>Поиск по каталогу</h2>
<? module('doc:read:catalog', $search) ?><? }else{ ?><? $module_data = array(); $module_data["parent*"] = "$id:catalog"; $module_data["type"] = "product"; $module_data["url"] = ""; moduleEx("doc:read:catalog", $module_data); ?><? } ?>
</div>
...
<?  if (beginCompile($data, "productPageImage")){ ?><? displayThumbImage($title = docTitleImage($id), array(250, 350), ' class="thumb"', '', $title) ?><? $module_data = array(); $module_data["src"] = "$folder/Gallery"; moduleEx("gallery:small", $module_data); ?><?  endCompile($data, "productPageImage"); } ?>
    </th>
    <td width="100%" valign="top">
    <? if(isset($price)) echo $price ?><? module("bask:button:$id"); ?><br />
...
<? endAdmin($menu, true) ?><? event('document.comment',	$id)?><? } ?><? //	Template doc_read_catalog loaded from  _modules/_module.doc/_pages/_reads/template.doc_read_catalog.php3 ?>
<?
...
<? foreach($table as &$data){
	...
	$price		= docPriceFormat2($data);
?>
<th><?  if (beginCompile($data, "catalogThumb2")){ ?><? if($id) displayThumbImage($title = docTitleImage($id), array(120, 150), '', '', $title); else echo '&nbsp;'; ?><?  endCompile($data, "catalogThumb2"); } ?></th>
...
<? if(isset($price)) echo $price ?><? module("bask:button:$id"); ?><? endAdmin($menu, true) ?><? }else echo '&nbsp;'; ?></td>
<? }//	foreach ?></tr>
<? }//	while ?>
</table>
<? if(isset($p)) echo $p ?><? return $search; } ?><? //	Template doc_read_default loaded from  _modules/_module.doc/_pages/_reads/template.doc_read_default.php3 ?>
<?
...
<? endAdmin($menu, true) ?><? } ?><? return $search; } ?><? //	Template doc_read_news loaded from  _modules/_module.doc/_pages/_reads/template.doc_read_news.php3 ?>
<?
...
<? } ?><? return $search; } ?><? //	Template doc_read_news2 loaded from  _modules/_module.doc/_pages/_reads/template.doc_read_news2.php3 ?>
<?
...
<? endAdmin($menu, true) ?><? } ?><? return $search; } ?><? //	Template doc_read_news3 loaded from  _modules/_module.doc/_pages/_reads/template.doc_read_news3.php3 ?>
<?
...
<? return $search; } ?><? //	Template doc_menuTools loaded from  _modules/_module.doc/_pages/_reads/_menu/template.doc_menuTools.php3 ?>
﻿<?
...
<? }else{ ?><? module("display:sort"); ?><? if(isset($p)) echo $p ?><? } ?><? } ?><? if (!$search) endCache($cache); ?><? } ?><? //	Template gallery_default loaded from  _modules/_module.gallery/template.gallery_default.php3 ?>
<?
...
<? }//	swith ?><? } ?><? //	Template feedback_display_vertical loaded from  _modules/_nodule.feedback/template.feedback_display_vertical.php3 ?>
<?
...
?><? //	Template doc_read_address loaded from  _sites/fs/_modules/_address/_read/template.doc_read_address.php3 ?>
<?
function doc_read_address_beginCache($db, $val, &$search)
{
	m('script:jq');
	if (userID()) return;
	return hashData($search);
}
function doc_read_address($db, $val, &$search)
{
	$table	= array();
	$data	= $db->next();
	$row	= 0;
...
?>
<table width="100%" cellpadding="10" cellpadding="0" class="parthers">
<? foreach($table as &$row){ ?>
<tr>
<? foreach($row as &$data){
	$db->data 	= $data;
...
<? foreach($row as &$data){
	$db->data 	= $data;
	$id			= $db->id();
	$title		= module("doc:titleImage:$id");
	if ($title){
		list($w, $h)= getimagesize($title);
		$title		= imagePath2local($title);
		$title = "<img src=\"$title\" width=\"$w\" height=\"$h\" />";
	}
	$link		= getURL($db->url());
?>
<th><a href="<? if(isset($link)) echo $link ?>" title="<? if(isset($data["title"])) echo htmlspecialchars($data["title"]) ?>"><? if(isset($title)) echo $title ?></a></th>
<? } ?>
</tr>
<tr>
<? foreach($row as &$data){
	...
	$note	= nl2br($fields['any']['note']);
	$address= nl2br($fields['any']['places']);
?>
<td valign="top">
<address><? if(isset($address)) echo $address ?></address>
<p><? if(isset($url)) echo $url ?></p>
<div class="note"><? if(isset($note)) echo $note ?></div>
</td>
<? } ?>
</tr>
<? } ?>
</table>
<? return $search; } ?><? //	Template doc_page_page_master loaded from  _sites/fs/_modules/_master/_page/template.doc_page_page_master.php3 ?>
﻿<? function doc_page_page_master($db, &$menu, &$data){
?><? beginAdmin() ?><? document($data) ?><? endAdmin($menu) ?><? $module_data = array(); $module_data["prop"]["!place"] = "master"; moduleEx("doc:read:master3", $module_data); ?><? } ?><? //	Template doc_read_article_master loaded from  _sites/fs/_modules/_master/_read/template.doc_read_article_master.php3 ?>
<? function doc_read_article_master($db, $val, $search){
	$data	= $db->data;
	$id		= $db->id();
	$link	= getURL($db->url());
	$menu	= doc_menu($id, $data, $false);
...
<div class="slot article">
    ...
    <h1><a href="<? if(isset($link)) echo $link ?>"><? if(isset($data["title"])) echo htmlspecialchars($data["title"]) ?></a></h1>
    <div class="holder">
<? beginAdmin() ?>
<div class="info bg">
<b>Приходите</b> <a href="<? if(isset($link)) echo $link ?>"><? if(isset($data["fields"]["any"]["place"])) echo htmlspecialchars($data["fields"]["any"]["place"]) ?></a>
</div>
        <div class="info2 bg2">
...
<? endAdmin($menu) ?>
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="accept">
<tr>
    <td class="bg"><a href="<? if(isset($link)) echo $link ?>">Узнать подробности</a></td>
    <th class="bg3"><a href="<? if(isset($link)) echo $link ?>">Хочу участвовать!</a></th>
</tr>
</table>
	</div>
</div><br>
<? } ?><? //	Template doc_read_master loaded from  _sites/fs/_modules/_master/_read/template.doc_read_master.php3 ?>
<?
function doc_read_master_beginCache($db, $val, &$search){
	if (userID()) return;
	return hashData($search);
}
function doc_read_master_before($db, $val, &$search){
	if (!isset($search[':order'])) $search[':order'] = 'masterDate';
}
function doc_read_master($db, $val, &$search){

	if (!$db->rows()) return $search;
	switch($db->rows()){
		case 1: read_master1($db); break;
		case 2: read_master2($db); break;
		case 3: read_master3($db); break;
		default:read_master4($db); break;
	}
	return $search;
}
?><? /********************************************/
function read_master1($db){
?>
<div class="slot big">
    <? $db->next(); masterInfo($db, 'design/m1.png')?>
</div>
<? } ?><? /*******************************************/
function read_master2($db){?>
<table width="100%" border="0" cellspacing="0" cellpadding="0">
<tr>
    <td valign="top" width="730">
<div class="slot big">
    <? $db->next(); masterInfo($db, 'design/m2.png')?>
</div>
</td>
<td valign="top" class="right" style="padding-left:10px">
<div class="slot">
    <? $db->next(); masterInfo($db, 'design/m3.png')?>
</div>
</td>
</tr>
</table>
<? } ?><? /*********************************************/
function read_master3($db){?>
<table width="100%" border="0" cellspacing="0" cellpadding="0">
<tr>
...
<? } ?><? /****************************************************/
function read_master4($db){?>
<table width="100%" border="0" cellspacing="0" cellpadding="0">
<tr>
    <td valign="top" class="left"><div class="slot">
    <? $db->next(); masterInfo($db, 'design/m3.png')?>
	</div></td>
    <td valign="top" class="center"><div class="slot">
    <? $db->next(); masterInfo($db, 'design/m3.png')?>
    </div></td>
    <td valign="top" class="right">
<? $split = ' id="first"'; for($ix=0; $ix<2; ++$ix){?>
    <div class="slot small" <? if(isset($split)) echo $split ?>>
    <? $db->next(); masterInfo($db, 'design/m4.png')?>
    </div>
<? $split=''; }// while 2 ?>
    </td>
</tr>
...
function masterInfo($db, $mask){
	...
	$menu	= doc_menu($id, $data, true);
?>
<a href="<? if(isset($link)) echo $link ?>" class="image"  title="<? if(isset($data["title"])) echo htmlspecialchars($data["title"]) ?>"><? $module_data = array(); $module_data["mask"] = "$mask"; $module_data["popup"] = "false"; moduleEx("doc:titleImage:$id:mask", $module_data); ?></a>
    <div class="date bg"><? if(isset($date)) echo $date ?></div>
    <div class="holder">
<? beginAdmin() ?>
        <address class="info bg"><a href="<? if(isset($link)) echo $link ?>"><? if(isset($data["fields"]["any"]["place"])) echo htmlspecialchars($data["fields"]["any"]["place"]) ?></a></address>
        <div class="info2 bg2">
        	<h2><a href="<? if(isset($link)) echo $link ?>" title="<? if(isset($data["title"])) echo htmlspecialchars($data["title"]) ?>"><? if(isset($data["title"])) echo htmlspecialchars($data["title"]) ?></a></h2>
            <p><a href="<? if(isset($link)) echo $link ?>"><? if(isset($data["fields"]["note"])) echo htmlspecialchars($data["fields"]["note"]) ?></a></p>
        </div>
<? endAdmin($menu) ?><? masterAccept($db) ?>
    </div>
<? } ?><? //	Template doc_read_master2 loaded from  _sites/fs/_modules/_master/_read/template.doc_read_master2.php3 ?>
<?
function doc_read_master2_beginCache($db, $val, &$search){
	return hashData($search);
}
function doc_read_master2($db, $val, &$search)
{
	if (!$db->rows()) return $search;
...
<?	$first = ' id="first"';
	...
	$id		= $db->id();
	$url 	= $db->url();
	$folder	= module("doc:titleImage:$id", 275);
	$class	= $folder?" style=\"background-image:url($folder)\"":'';
?>
<td width="<? if(isset($w)) echo htmlspecialchars($w) ?>%" <? if(isset($first)) echo $first ?>>
<div class="holder">
<h3><a href="<? module("url:$url"); ?>"><? if(isset($data["title"])) echo htmlspecialchars($data["title"]) ?></a></h3>
<div <? if(isset($class)) echo $class ?>></div>
<div class="menu"><? $module_data = array(); $module_data["parent"] = "$id"; moduleEx("doc:read:menu", $module_data); ?></div>
</div>
</td>
<? $first = ''; } ?>
</tr>
</table>
<? return $search; } ?><? //	Template doc_read_master3 loaded from  _sites/fs/_modules/_master/_read/template.doc_read_master3.php3 ?>
﻿<?
function doc_read_master3_beginCache($db, $val, &$search){
	if (userID()) return;
	return hashData($search);
}
function doc_read_master3_before($db, $val, &$search){
	if (!isset($search[':order'])) $search[':order'] = 'masterDate';
}
function doc_read_master3($db, $val, &$search){
?><? while($data = $db->next()){
	$id		= $db->id();
	$link	= getURL($db->url());
	$date	= masterDate($data);
...
?>
<div class="timeline">
<h4><? if(isset($date)) echo $date ?></h4>
<? beginAdmin() ?>
<h2><a href="<? if(isset($link)) echo $link ?>"><? if(isset($data["title"])) echo htmlspecialchars($data["title"]) ?></a></h2>
<blockquote>
<address><? if(isset($data["fields"]["any"]["place"])) echo htmlspecialchars($data["fields"]["any"]["place"]) ?></address>
<? if(isset($data["fields"]["note"])) echo htmlspecialchars($data["fields"]["note"]) ?>
</blockquote>
<? endAdmin($menu) ?>
</div>
<? } ?><? } ?>