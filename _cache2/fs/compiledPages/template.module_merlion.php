...
?>
<form action="<? module("getURL:import_merlion"); ?>" method="post">
...
?><option value="<? if(isset($code)) echo htmlspecialchars($code) ?>"<? if(isset($class)) echo $class ?>><? if(isset($val["Description"])) echo htmlspecialchars($val["Description"]) ?></option><? } ?>
</select>
    </td>
  </tr>
  <tr>
...
?><option value="<? if(isset($Date)) echo htmlspecialchars($Date) ?>"<? if(isset($class)) echo $class ?>><? if(isset($Date)) echo htmlspecialchars($Date) ?></option><? } ?>
</select>
    </td>
  </tr>
  <tr>
...
foreach($m as $Code => $val){
	$class = $thisValue == $Code?' selected="selected" class="current"':'';
?><option value="<? if(isset($Code)) echo htmlspecialchars($Code) ?>:<? if(isset($val)) echo htmlspecialchars($val) ?>"<? if(isset($class)) echo $class ?>><? if(isset($Code)) echo htmlspecialchars($Code) ?>: <? if(isset($val)) echo htmlspecialchars($val) ?></option><? } ?>
</select>
    </td>
    </tr>
</table>
...
<? foreach($childs as $parentID){
	...
	if (!$price) $price = '';
?>
<tr <? if(isset($class2)) echo $class2 ?>>
    <td class="deep<? if(isset($deep)) echo htmlspecialchars($deep) ?>">
<input type="hidden" name="merlionCatalog[<? if(isset($parentID)) echo htmlspecialchars($parentID) ?>]" value="0" />
<input type="checkbox" name="merlionCatalog[<? if(isset($parentID)) echo htmlspecialchars($parentID) ?>]" value="1" <? if(isset($class)) echo $class ?> />
<a href="<? if(isset($link)) echo $link ?>" id="ajax"><? if(isset($name)) echo htmlspecialchars($name) ?></a>
    </td>
    <td><a href="<? if(isset($link2)) echo $link2 ?>" id="ajax">цены</a></td>
    <td>
<input type="text" class="input w00" name="merlionPrice[<? if(isset($parentID)) echo htmlspecialchars($parentID) ?>]" value="<? if(isset($price)) echo htmlspecialchars($price) ?>"  />
    </td>
    <td><? if ($data){ ?><input name="merlionDelete[<? if(isset($parentID)) echo htmlspecialchars($parentID) ?>]" type="checkbox" value="<? if(isset($data)) echo htmlspecialchars($data) ?>" /><? } ?></td>
</tr>
...
?>
  <tr>
    <td><? if(isset($brand)) echo htmlspecialchars($brand) ?></td>
    <td><a href="<? if(isset($link)) echo $link ?>" id="ajax"><? if(isset($name)) echo htmlspecialchars($name) ?></a></th>
  </tr>
<? } ?>
</table>
<? } ?>
...
?>
<a href="<? if(isset($url)) echo $url ?>" id="ajax">Каталог</a>
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="table">
  <tr>
...
?>
  <tr>
    <td><a href="http://img.merlion.ru/items/<? if(isset($name)) echo htmlspecialchars($name) ?>" target="_blank"><? if(isset($name)) echo htmlspecialchars($name) ?></a></td>
    <td><? if(isset($type)) echo htmlspecialchars($type) ?></td>
    <td><? if(isset($size)) echo htmlspecialchars($size) ?></td>
  </tr>
<? } ?>
</table>
<? } ?>
...
<? foreach($item as $name => $val){ ?>
    <th><? if(isset($name)) echo htmlspecialchars($name) ?></th>
<? } ?>
  </tr>
<? foreach($xml as &$item){ ?>
  <tr>
<? foreach($item as $name => &$val){ ?>
    <td><? if(isset($val)) echo htmlspecialchars($val) ?></td>
<? } ?>
  </tr>
<? } ?>
</table>
...
<p>
<h2>Мерлион</h2>
<a href="<? module("getURL:import_merlion"); ?>">Каталоги</a> <a href="<? module("getURL:import_merlion_synch"); ?>">Товары</a>
</p>
<? } ?>
