...
<div id="seoSEO" class="ui-tabs-panel ui-widget-content ui-corner-bottom">
    ...
    <div><input name="SEO[title]" type="text" value="<? if(isset($SEO["title"])) echo htmlspecialchars($SEO["title"]) ?>" class="input w100" /></div>
    Ключевые слова (keywords metatag)
    <div><input name="SEO[keywords]" type="text" value="<? if(isset($SEO["keywords"])) echo htmlspecialchars($SEO["keywords"]) ?>" class="input w100" /></div>
    Описание (description metatag)
    <div><textarea name="SEO[description]" cols="" rows="5" class="input w100"><? if(isset($SEO["description"])) echo htmlspecialchars($SEO["description"]) ?></textarea></div>
    <div>Аннотация. Подпись в меню, если задано в дизайне</div>
    <div><textarea name="doc[fields][note]" cols="" rows="4" class="input w100"><? if(isset($fields["note"])) echo htmlspecialchars($fields["note"]) ?></textarea></div>
    <div>Класс стиля ссылки на страницу (пример: <b>icon i12</b>)</div>
    <div><input name="doc[fields][class]" type="text" class="input w100" value="<? if(isset($fields["class"])) echo htmlspecialchars($fields["class"]) ?>" size="" /></div>
</div>
<div id="seoTAGS" class="ui-tabs-panel ui-widget-content ui-corner-bottom">
    ...
    <tr>
        ...
        <td width="100%"><input name="SEO[<? if(isset($name)) echo htmlspecialchars($name) ?>]" type="text" value="<? if(isset($val)) echo htmlspecialchars($val) ?>" class="input w100" /></td>
    </tr>
    <? } ?>
...
?>
    ...
    <div><input name="doc[fields][redirect]" type="text" value="<? if(isset($fields["redirect"])) echo htmlspecialchars($fields["redirect"]) ?>" class="input w100" /></div><br />
    ...
    <div><input type="text" name="documetntLinks[]" class="input w100" value="<? if(isset($link)) echo htmlspecialchars($link) ?>" /></div>
    <? } ?>
