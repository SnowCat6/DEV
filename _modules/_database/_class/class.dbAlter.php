...
class dbAlter
{
	...
	static function alterTable($table, $fields, $dbEngine = '', $rowFormat = '')
	{
		$dbLink	= new dbRow();
		$dbLink	= $dbLink->dbLink;
		$dbLink->connect(true);
	...
	//define('_debug_', true);
		...
		if ($rs)
		{
			...
			$keys	= dbAlter::dbParseKeys($create);
			//	Database engine
			$thisEngine		= dbAlter::dbParseValue('ENGINE',	$create);
			if ($thisEngine != $dbEngine){
				$dbLink->dbExec("ALTER TABLE `$table` ENGINE=$dbEngine");;
			}
			//	Database row format
			$thisRowFormat	= dbAlter::dbParseValue('ROW_FORMAT', $create);
			...
			while($data = $dbLink->dbResult($rs))
			{
				...
				$f['Field']	= $name;
				dbAlter::dbAlterCheckField($alter["CHANGE COLUMN `$name` `$name`"], $f, $data, false, $keys);
			}
			//	Добавить несуществующие колонки
			foreach($fields as $name => $f){
				$data 		= array();
				$f['Field'] = $name;
				dbAlter::dbAlterCheckField($alter["ADD COLUMN `$name`"], $f, $data);
			}
			//	Создать SQL запрос
			$sql = array();
	...
	//		$dbLink->dbExec("OPTIMIZE TABLE $table");
		...
		foreach($fields as $name => $f)
		{
			...
			$data 		= array();
			$f['Field'] = $name;
			dbAlter::dbAlterCheckField($alter["`$name`"], $f, $data, true);
		}
		$sql = array();
		//	Создать SQL
		foreach($alter as $name=>$value){
	...
	static function dbAlterCheckField(&$alter, &$need, &$now, $bCreate = false, $keys = NULL)
	{	
	...
	static function dbParseValue($name, $code)
	{
		if (!preg_match("#$name\s*=\s*([^\s]+)#", $code, $var)) return NULL;
		return $var[1];
	}
	static function dbParseKeys($code)
	{
