...
class  IDN{
    ...
    public static function decodeIDN_URL($name)
	{
		if (preg_match('#^(\w+://)([^/\?\#]+)(.*)#', $name, $var)){
			return $var[1].IDN::decodeIDN($var[2]).$var[3];
		}
		if (preg_match('#^([^/\?\#]+)(.*)#', $name, $var)){
			return IDN::decodeIDN($var[1]).$var[2];
		}
		return IDN::decodeIDN($name);
	}
}
?>